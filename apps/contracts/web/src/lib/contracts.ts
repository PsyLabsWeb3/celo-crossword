...
import { CrosswordBoardABI } from './abis/CrosswordBoard';
import { CrosswordPrizesABI } from './abis/CrosswordPrizes';
...
// Helper function to get contracts based on the network
function getContractsForNetwork(network: string) {
  switch (network) {
    case 'sepolia':
      return SEPOLIA_CONTRACTS;
    case 'alfajores':
      return ALFAJORES_CONTRACTS;
    case 'celo':
...
// Export the contracts for the current network (can be configured based on environment)
const CURRENT_NETWORK = process.env.NEXT_PUBLIC_NETWORK || 'sepolia';
export const CONTRACTS = getContractsForNetwork(CURRENT_NETWORK);

// Also export specific contract addresses
export const CROSSWORD_BOARD_ADDRESS = CONTRACTS.CrosswordBoard.address;
export const CROSSWORD_PRIZES_ADDRESS = CONTRACTS.CrosswordPrizes.address;